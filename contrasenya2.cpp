...
int t, num=0, lletra=0, mayu=0;//inicialitza les variables
    ...
    if(strlen(password)<8){ //si la longitud de la contrasenya es mes petit que 8 entra
        ...
        if(num==0 || lletra==0 || mayu==0){ //si les variables son igual a 0 vol dir que no ha complert lo d'abans i llavors printara el seguent
            printf("ERROR ERROR hi ha d'haver-hi numeros, lletres minusculas i mayusculas ");
    	}
    }
